...
function Opt({
  ...
  badge,
  onClick,
}: { title: string; sub?: string; badge?: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      <span>
        {badge && (
          <span className="mb-1.5 inline-block rounded-full bg-brand px-2 py-0.5 text-[10px] font-bold text-slate-950">
            ⭐ {badge}
          </span>
        )}
      ...
      <span className="shrink-0 text-lg text-slate-500 transition group-hover:translate-x-0.5 group-hover:text-brand">
        →
      </span>
    </button>
  );
...
export function StartWizard({
  ...
  plans,
  freeHref,
}: { dict: Dict; plans: Plans; freeHref: string }) {
    ...
    <div className="w-full max-w-lg">
      ...
      {step === "audience" && (
        <div className="mt-6 space-y-3">
          <Opt
            title={dict.q1Business}
            sub={dict.q1BusinessSub}
            onClick={() => {
          ...
          />
          <Opt
            title={dict.q1Personal}
            sub={dict.q1PersonalSub}
            onClick={() => {
      ...
      {step === "bizSize" && (
        <div className="mt-6 space-y-3">
          ...
          />
          <Opt
            title={dict.bizUpTo200}
            badge={dict.mostChosen}
            onClick={() => {
              setPlanId("scale");
              go("bizPerk");
            }}
      ...
      {step === "famSize" && (
        <div className="mt-6 space-y-3">
          ...
          />
          <Opt
            title={dict.famUpTo10}
            badge={dict.mostChosen}
            onClick={() => {
              setPlanId("familyplus");
              go("reflect");
            }}
          />
          <Opt
            title={dict.famJustMe}
            onClick={() => {
              setPlanId("free");
              go("result");
            }}
      ...
      {step === "result" && planId === "free" && (
        <div className="mt-6 rounded-2xl border border-emerald-500/30 bg-emerald-500/[0.06] p-6 text-center">
          ...
          <Link
            href={freeHref}
            className="mt-5 block rounded-xl bg-emerald-500 px-5 py-3 text-sm font-bold text-slate-950 transition hover:bg-emerald-400"
          >
            {dict.freeStart}
          </Link>
      ...
      {step === "result" && planId && planId !== "free" && (
        <div className="mt-6 rounded-2xl border border-brand/40 bg-white/[0.03] p-6 text-center">
          ...
          {plans[planId].priceSub && (
            <p className="mt-1 text-xs text-slate-400">{plans[planId].priceSub}</p>
          )}
          <a
            href={plans[planId].href}
            target="_blank"
            rel="noreferrer"
          ...
          {audience === "business" && (
            <p className="mt-3 rounded-lg border border-brand/20 bg-brand/[0.06] px-3 py-2 text-xs text-brand">
              🎁 {dict.perkLine}
            </p>
          )}
        </div>
