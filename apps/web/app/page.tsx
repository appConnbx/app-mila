    ...
    <div className="glass glow-top glow-blue relative p-3 sm:p-4">
      ...
      <div className="flex items-center justify-between border-b border-white/5 pb-3">
        <div className="flex items-center gap-2">
          <span className="grid h-6 w-6 place-items-center rounded-md bg-brand text-[11px] font-black text-slate-950">
            M
          </span>
      ...
      <div className="mt-3 space-y-1.5">
        {[
          ...
          >
            ...
            <span className="ml-2 shrink-0 rounded-full bg-brand/10 px-2 py-0.5 text-[10px] font-medium text-brand">
              {d.s}
            </span>
          </div>
        ))}
    ...
    <div className={MOCK_FRAME}>
      <div className="relative h-full w-full rounded-xl border border-white/5 bg-gradient-to-br from-slate-900 to-slate-950">
        ...
        <div className="absolute right-0 top-1/2 flex -translate-y-1/2 flex-col items-center gap-1.5 rounded-l-2xl border border-white/10 bg-white/[0.06] px-2 py-2.5 backdrop-blur">
          <span className="grid h-6 w-6 place-items-center rounded-md bg-brand text-[10px] font-black text-slate-950">
            M
          </span>
          <span className="rounded-full bg-rose-500/80 px-1.5 text-[9px] font-bold text-white">
            3
          </span>
        ...
        <div className="absolute right-12 top-1/2 w-44 -translate-y-1/2 rounded-xl border border-white/10 bg-slate-900/90 p-3 shadow-xl">
          ...
          <div className="mt-2 flex items-center gap-1">
            <span className="rounded-full bg-white/10 px-1.5 py-0.5 text-[9px] text-slate-300">
              {t("mockup.kpiInProgress")}
            </span>
            <span className="rounded-full bg-brand/10 px-1.5 py-0.5 text-[9px] text-brand">
              amanhã
            </span>
          </div>
        </div>
        ...
        <div className="space-y-2">
          {rows.map((r, i) => (
            <div key={i} className="rounded-lg border border-white/5 bg-white/[0.03] px-2 py-2">
              ...
              <span className="mt-1 inline-block rounded-full bg-brand/10 px-1.5 text-[9px] text-brand">
                {r.s}
              </span>
            </div>
          ))}
...
export default async function LandingPage() {
  ...
  const bold = {
    b: (chunks: ReactNode) => <span className="font-semibold text-white">{chunks}</span>,
  };
  ...
  const corpPlans = [
    {
      name: "Starter",
      users: t("plans.starterUsers"),
      intl: "87",
      brParcela: "297",
      brTotal: "3.564",
      href: checkout("wyitwc3d", "starter"),
    },
    {
      name: "Growth",
      users: t("plans.growthUsers"),
      intl: "167",
      brParcela: "497",
      brTotal: "5.964",
      href: checkout("2kxlbff2", "growth"),
    },
    {
      name: "Scale",
      users: t("plans.scaleUsers"),
      intl: "337",
      brParcela: "697",
      brTotal: "8.364",
      href: checkout("abpzxjap", "scale"),
      featured: true,
    },
    {
      name: "Enterprise",
      users: t("plans.enterpriseUsers"),
      intl: "667",
      brParcela: "1.117",
      brTotal: "13.404",
      href: checkout("yl7fpa6u", "enterprise"),
    },
  ];
  const familyPlans = [
    {
      name: "Family",
      users: t("plans.familyUsers"),
      intl: "13",
      brAnnual: "97",
      brParcela: "8,08",
      href: checkout("i67ovflk", "family"),
    },
    {
      name: "Family Plus",
      users: t("plans.familyPlusUsers"),
      intl: "17",
      brAnnual: "127",
      brParcela: "10,58",
      href: checkout("tfkn6adh", "family_plus"),
      featured: true,
    },
  ];
    ...
    if (!isBR)
      return (
        <p className="mt-4 text-3xl font-extrabold text-white">
          {cur}
          {p.intl}
          <span className="text-base font-medium text-slate-400">{perMonth}</span>
        </p>
      );
    if (p.brAnnual)
      return (
      ...
      <header className="sticky top-0 z-50 border-b border-white/5 bg-surface/80 backdrop-blur">
        <nav className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3.5">
          ...
          <div className="hidden items-center gap-7 text-sm text-slate-300 md:flex">
            ...
            <a href="#pessoal" className="transition hover:text-white">
              {t("nav.personal")}
            </a>
            <a href="#afiliados" className="transition hover:text-white">
              {t("nav.affiliates")}
            </a>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Link
              href="/login"
              className="hidden rounded-lg px-3 py-2 text-sm font-medium text-slate-300 transition hover:text-white sm:inline-block"
            >
              {t("nav.signIn")}
            </Link>
            <Link
      ...
      <section id="praticidade" className="mx-auto max-w-6xl px-4 py-12">
        ...
        <div className="mt-10 grid items-start gap-6 md:grid-cols-3">
          <figure className="space-y-3">
            <GadgetMockup />
            <figcaption className="text-center text-sm font-medium text-slate-300">
              {t("eco.capWindows")}
            </figcaption>
          </figure>
          <figure className="space-y-3">
          ...
          <div className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {corpPlans.map((p) => (
              ...
              >
                ...
                {priceBlock(p)}
                {p.featured && (
                  <p className="mt-1.5 text-xs font-medium text-brand">{t("plans.corpPopular")}</p>
                )}
                <a
                  href={p.href}
                  target="_blank"
        ...
        <div className="mt-12">
          ...
          <div className="mt-5 flex flex-col items-start justify-between gap-3 rounded-xl border border-white/10 bg-white/[0.02] p-4 sm:flex-row sm:items-center">
            <div className="flex items-center gap-2 text-sm text-slate-400">
              ...
              <span>
                <b className="text-slate-200">{t("plans.freeName")}</b> — {t("plans.freeDesc")}
              </span>
            </div>
            <Link
        ...
        <div className="glass glow-top relative overflow-hidden border border-amber-400/20 p-8 sm:p-12">
          ...
          <div className="relative grid items-center gap-6 lg:grid-cols-[1.4fr,1fr]">
            ...
            <div className="lg:justify-self-end">
              ...
              <p className="mt-2 text-center text-xs font-semibold text-amber-300/90 lg:text-right">
                {t("affiliates.soon")}
              </p>
            </div>
          </div>
      ...
      <footer className="border-t border-white/5">
        ...
        <div className="border-t border-white/5">
          <div className="mx-auto max-w-6xl space-y-2 px-4 py-6 text-xs text-slate-500">
            ...
            <p>
              {t("footer.support")}:{" "}
              <a href="mailto:help@appmila.co" className="text-slate-400 hover:text-white">
                help@appmila.co
              </a>
              {" · "}
              {t("footer.sales")}:{" "}
              <a href="mailto:sales@appmila.co" className="text-slate-400 hover:text-white">
                sales@appmila.co
              </a>
              {" · "}
              {t("footer.corporate")}:{" "}
              <a href="mailto:mail@connbx.com.br" className="text-slate-400 hover:text-white">
                mail@connbx.com.br
              </a>
            </p>
