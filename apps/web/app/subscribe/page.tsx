...
const toLocale = (l?: string) =>
  l === "en" ? "en" : l === "es" ? "es" : l === "pt-BR" ? "pt-BR" : null;
...
export default async function SubscribePage({
  ...
  async function start(formData: FormData) {
    ...
    const back = (q: string) =>
      `/subscribe?plan=${encodeURIComponent(p)}&lang=${encodeURIComponent(ln)}&next=${encodeURIComponent(nextPath)}&${q}`;
    if (!VALID.has(p) || !email) redirect(back("erro=1"));
    // Rate-limit por IP: cada tentativa abre sessão de checkout no Stripe.
    if (await rateLimit(`subscribe:${await clientIp()}`, { windowMs: 600_000, max: 8 }))
          ...
          <>
            <div className="mb-5 text-center">
              ...
              <p className="mt-1 text-sm text-brand">
                {t("subPlanLine", { plan: PLAN_LABEL[plan], price: PLAN_USD[plan] })}
              </p>
            </div>
            {erro && (
            ...
            <form action={start} className="space-y-3">
              ...
              <div>
                <label htmlFor="sub-email" className="mb-1 block text-sm text-slate-300">
                  {t("subEmailLabel")}
                </label>
                <input
                  id="sub-email"
