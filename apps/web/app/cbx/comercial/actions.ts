...
export async function addNote(formData: FormData) {
  ...
  const supabase = await createClient();
  const { data } = await (supabase as unknown as Rpc).rpc("cbx_add_client_note", {
    p_holding: holding,
    p_body: body,
  });
  revalidatePath(`/cbx/comercial/${holding}`);
...
export async function setLicense(formData: FormData) {
  ...
  revalidatePath(`/cbx/comercial/${holding}`);
  redirect(
    `/cbx/comercial/${holding}?${data?.ok ? "ok=licenca" : `err=${data?.reason ?? "erro"}`}`,
  );
}
...
export async function cbxSetUserActive(formData: FormData) {
  ...
  const supabase = await createClient();
  await (supabase as unknown as Rpc).rpc("cbx_set_person_active", {
    p_person: person,
    p_active: active,
  });
  revalidatePath(`/cbx/comercial/${holding}`);
...
export async function cbxSetUserPassword(formData: FormData) {
  ...
  const sb = supabase as unknown as {
    rpc: (
      ...
      data: { auth_user_id?: string | null; email?: string | null; forbidden?: boolean } | null;
    }>;
  };
  ...
  if (data.auth_user_id) {
    ...
    if (!uid) {
      ...
      const { data: existing } = await sbAdmin.rpc("auth_user_id_by_email", {
        p_email: data.email,
      });
      uid = existing ?? null;
