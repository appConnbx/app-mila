  ...
  return (
    <form action={action} className="space-y-3">
      ...
      <div>
        ...
        <select name="business_type" defaultValue={initial.business_type ?? ''} className={`mt-1 ${inputCbx}`}>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <div>
          ...
          {isBR ? (
            ...
            <input name="state" defaultValue={initial.state ?? ''} placeholder="Estado/Província" className={`mt-1 ${inputCbx}`} />
          )}
        </div>
        <div>
          <label className={labelCbx}>Cidade</label>
          <input name="city" defaultValue={initial.city ?? ''} placeholder="Cidade" className={`mt-1 ${inputCbx}`} />
        </div>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div>
          ...
          <input name="contact_name" defaultValue={initial.contact_name ?? ''} className={`mt-1 ${inputCbx}`} />
        </div>
        <div>
          <label className={labelCbx}>E-mail do contato</label>
          <input name="contact_email" type="email" defaultValue={initial.contact_email ?? ''} className={`mt-1 ${inputCbx}`} />
        </div>
        <div>
          <label className={labelCbx}>Telefone</label>
          <input name="contact_phone" defaultValue={initial.contact_phone ?? ''} className={`mt-1 ${inputCbx}`} />
        </div>
      </div>
