...
import { Pill, btnGhostCbx, fmtDate, tdCbx, thCbx } from "../_ui";
...
export function InstanceUsers({
  ...
  rows,
  canManage,
}: { holdingId: string; rows: InstanceUserRow[]; canManage: boolean }) {
  ...
  const filtered = useMemo(() => {
    ...
    return s
      ? rows.filter(
          (r) => r.full_name.toLowerCase().includes(s) || (r.email ?? "").toLowerCase().includes(s),
        )
      : rows;
  }, [q, rows]);
  ...
  const sorted = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      ...
      if (sortKey === "lastlogin")
        return (
          ((a.last_sign_in_at ? Date.parse(a.last_sign_in_at) : 0) -
            (b.last_sign_in_at ? Date.parse(b.last_sign_in_at) : 0)) *
          m
        );
          ...
          <tbody>
            {pageRows.map((u) => (
              ...
              >
                ...
                <td className={tdCbx}>
                  {u.streak > 0 ? (
                    <span className="font-semibold text-amber-300">🔥 {u.streak}</span>
                  ) : (
                    <span className="text-slate-600">—</span>
                  )}
                </td>
                <td className={tdCbx}>
                  <Pill tone={u.is_active ? "ok" : undefined}>
        ...
        >
          ...
          <div className="relative w-full max-w-md rounded-2xl border border-white/10 bg-slate-900 p-6">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {open.email ?? "sem e-mail"} · {open.has_login ? "tem acesso" : "sem login"}
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-3 flex items-center gap-2">
              <Pill tone={open.is_active ? "ok" : undefined}>
                {open.is_active ? "Ativo" : "Inativo"}
              </Pill>
            ...
            {canManage ? (
              <div className="mt-5 space-y-4">
                ...
                {open.email ? (
                  <form
                    action={cbxSetUserPassword}
                    className="space-y-2 rounded-xl border border-white/10 bg-slate-900/60 p-3"
                  >
                    <input type="hidden" name="holding_id" value={holdingId} />
                    <input type="hidden" name="person_id" value={open.id} />
                    <label
                  ...
                  <p className="text-xs text-slate-500">
                    Cadastre um e-mail para este usuário antes de definir a senha.
                  </p>
                )}
              </div>
            ) : (
