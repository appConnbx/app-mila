...
function fmtMoney(cents: number, currency: string | null, locale: string): string {
  if (!cents) return "—";
  try {
    return new Intl.NumberFormat(locale, { style: "currency", currency: currency || "BRL" }).format(
      cents / 100,
    );
  } catch {
...
export function ComercialList({
  clients,
  locale = "pt-BR",
}: { clients: ClientRow[]; locale?: string }) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <span className="rounded-lg border border-white/10 bg-white/[0.04] px-3 py-1.5 text-sm text-slate-300">
          Faturado estim. (filtro):{" "}
          <b className="text-amber-300">{fmtMoney(totalEst, "BRL", locale)}</b>
        </span>
      </div>
          ...
          <tbody>
            {rows.map((c) => (
              ...
              >
                <td className={td}>
                  <Link
                    href={`/cbx/comercial/${c.holding_id}`}
                ...
                <td
                  className={`${td} font-semibold ${estRevenueCents(c) > 0 ? "text-amber-200" : "text-slate-500"}`}
                >
                ...
                <td className={td}>
                  <span
                    className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${c.sub_status === "active" ? "bg-emerald-500/15 text-emerald-300" : c.sub_status ? "bg-amber-500/15 text-amber-300" : "bg-rose-500/15 text-rose-300"}`}
                  >
      ...
      <p className="text-xs text-slate-600">
        Faturado estim. = preço do plano × períodos desde o cadastro (estimativa; ainda sem captura
        de valores reais de pagamento).
      </p>
    </div>
  );
}
