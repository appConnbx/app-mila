...
const FLASH: Record<string, { ok?: string; err?: string }> = {
  ...
  plan_kind_mismatch: { err: 'O plano escolhido não é do mesmo tipo (corporativo/família) da conta.' },
  plan_not_found: { err: 'Plano inválido.' },
  erro: { err: 'Não deu certo. Tente novamente.' },
}

export default async function NovoClientePage({ searchParams }: { searchParams: Promise<{ err?: string }> }) {
        ...
        <form action={createClientAccount} className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              ...
              <input name="name" required className={`mt-1 ${inputCbx}`} placeholder="Ex.: Grupo Exemplo" />
            </div>
            <div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">Em branco = limite do plano. VIP CONNBX é sempre ilimitado.</p>
            </div>
          </div>
