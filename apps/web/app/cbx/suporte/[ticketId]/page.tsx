...
export default async function TicketPage({
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
        ...
        <form action={updateTicket} className="grid gap-3 sm:grid-cols-3">
          ...
          <div>
            ...
            <select name="assignee" defaultValue="" className={`mt-1 ${inputCbx}`}>
              <option value="">{t.assignee_name ? `(manter: ${t.assignee_name})` : '— sem responsável —'}</option>
              {staff.map((s) => (
      ...
      {/* Comentários */}
      <CbxCard title="Comentários internos">
        <form action={addTicketComment} className="mb-4 flex gap-2">
          <input type="hidden" name="id" value={t.id} />
        ...
        <ul className="space-y-2">
          {comments.map((c) => (
            <li key={c.id} className="rounded-lg bg-slate-900/40 px-3 py-2">
              <p className="text-sm text-slate-200">{c.body}</p>
              <p className="mt-1 text-xs text-slate-500">{c.author_name} · {fmtDate(c.created_at)}</p>
            </li>
          ))}
          {comments.length === 0 && <li className="text-sm text-slate-500">Nenhum comentário ainda.</li>}
        </ul>
      </CbxCard>
    </div>
  )
