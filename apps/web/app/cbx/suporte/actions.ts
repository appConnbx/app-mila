...
export async function updateTicket(formData: FormData) {
  ...
  if (!id) return;
  const supabase = await createClient();
  await (supabase as unknown as Rpc).rpc("cbx_update_ticket", {
    p_id: id,
    p_status: status,
    p_assignee: assignee,
  });
...
export async function addTicketComment(formData: FormData) {
  ...
  const audience =
    String(formData.get("audience") ?? "internal") === "client" ? "client" : "internal";
  if (!id || !body) return;
  const supabase = await createClient();
  await (supabase as unknown as Rpc).rpc("cbx_add_ticket_comment", {
    p_id: id,
    p_body: body,
    p_audience: audience,
  });
...
export async function createSupportAccess(formData: FormData) {
  ...
  const hours = Math.min(
    Math.max(Number.parseInt(String(formData.get("hours") ?? "4"), 10) || 4, 1),
    24,
  );
...
export async function revokeSupportAccess(formData: FormData) {
  ...
  if (!id) return;
  const supabase = await createClient();
  const { data } = await (supabase as unknown as Rpc).rpc("cbx_revoke_support_access", {
    p_id: id,
  });
