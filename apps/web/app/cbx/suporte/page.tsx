...
export default async function CbxSuportePage({
  searchParams,
}: { searchParams: Promise<{ ok?: string; err?: string; view?: string }> }) {
  ...
  const clientMap = new Map<
    string,
    Map<string, { label: string; sort: number; items: Ticket[] }>
  >();
    ...
    return (
      ...
      >
        ...
        <td className={`${tdCbx} text-slate-400`}>
          {fmtDate(tab === "concluidos" ? (t.resolved_at ?? t.created_at) : t.created_at)}
        </td>
      </tr>
    );
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">
            {open.length} ticket(s) em aberto · {resolved.length} concluído(s).
          </p>
        </div>
      ...
      <div className="inline-flex rounded-lg border border-white/10 bg-white/[0.03] p-1">
        <Link
          href="/cbx/suporte"
          className={`rounded-md px-3 py-1 text-sm transition ${tab === "abertos" ? "bg-white/10 text-white" : "text-slate-400 hover:text-white"}`}
        >
          Abertos ({open.length})
        </Link>
        <Link
          href="/cbx/suporte?view=concluidos"
          className={`rounded-md px-3 py-1 text-sm transition ${tab === "concluidos" ? "bg-white/10 text-white" : "text-slate-400 hover:text-white"}`}
        >
