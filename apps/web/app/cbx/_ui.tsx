...
export function Kpi({
  ...
  value,
  tone,
}: { label: string; value: React.ReactNode; tone?: "ok" | "warn" }) {
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3">
      <p className="text-[11px] uppercase tracking-wide text-slate-500">{label}</p>
      <p
        className={`mt-1 text-2xl font-bold ${tone === "ok" ? "text-emerald-300" : tone === "warn" ? "text-amber-300" : "text-white"}`}
      >
        {value}
      </p>
    </div>
...
export function Pill({
  children,
  tone,
}: { children: React.ReactNode; tone?: "ok" | "warn" | "err" | "info" }) {
  const cls =
    tone === "ok"
