...
export const UL = ({ children }: { children: ReactNode }) => (
  <ul className="mt-4 list-disc space-y-2 pl-5 text-[15px] leading-relaxed text-slate-300 marker:text-brand">
    {children}
  </ul>
);
export const OL = ({ children }: { children: ReactNode }) => (
  <ol className="mt-4 list-decimal space-y-2 pl-5 text-[15px] leading-relaxed text-slate-300 marker:font-bold marker:text-brand">
    {children}
  </ol>
);
      ...
      <article className="mx-auto max-w-3xl px-4 py-12">
        ...
        <div className="mt-10 rounded-2xl border border-brand/30 bg-brand/10 p-6">
          <p className="text-lg font-semibold text-white">
            Experimente capturar sua próxima tarefa por voz
          </p>
          <p className="mt-1 text-sm text-slate-300">
            O appMila tem sistema web, app de celular e agente para computador. Comece grátis.
          </p>
          <div className="mt-4 flex flex-wrap gap-3">
            <Link
              href="/"
              className="rounded-lg bg-brand px-5 py-2.5 text-sm font-semibold text-slate-950 hover:bg-brand-500"
            >
              Conhecer o appMila
            </Link>
            <Link
              href="/start-family-free"
              className="rounded-lg border border-white/15 px-5 py-2.5 text-sm font-semibold text-slate-200 hover:bg-white/5"
            >
              Começar grátis (família)
            </Link>
          </div>
        </div>
