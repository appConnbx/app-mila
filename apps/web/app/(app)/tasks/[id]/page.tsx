...
import { addObservation, deleteDemand, setDemandStatus } from "../actions";
...
export default async function DemandDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: demandData } = await supabase
    .from("demands")
    .select(
      "id, title, description, status, priority, due_date, created_at, channel, responsible_id, origin_id, event_id, visibility, responsible:responsible_id(full_name), origin:origin_id(full_name), event:event_id(name)",
    )
    .eq("id", id)
    .single();
  ...
  const tz =
    (hRes.data as unknown as { timezone: string | null } | null)?.timezone ?? "America/Sao_Paulo";
  ...
  const PropCard = ({ label, value }: { label: string; value: string }) => (
    <div className="rounded-xl border border-surface-border bg-slate-900/40 px-4 py-3">
      ...
      <dd className="mt-1 truncate text-sm font-medium text-slate-100" title={value}>
        {value}
      </dd>
    </div>
  );
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <Link href="/tasks" className="text-sm text-slate-400 transition hover:text-white">
        ← {t("back")}
      </Link>
      ...
      <section className="mt-3 glass p-5">
              ...
              <SubmitButton
                className={`rounded-lg px-3 py-1.5 text-sm font-medium transition ${
                  d.status === s
                    ...
                    : "border border-surface-border text-slate-300 hover:bg-slate-800"
                }`}
              >
                {td(`status.${s}`)}
              </SubmitButton>
        ...
        <h2 className="mt-5 border-t border-surface-border pt-5 text-sm font-medium text-slate-400">
          {t("properties")}
        </h2>
        ...
        <dl className="mt-3 flex flex-wrap gap-x-6 gap-y-1 text-xs text-slate-500">
          <div className="flex gap-1.5">
            <dt>{t("origin")}:</dt>
            <dd className="text-slate-400">{d.origin?.full_name ?? "—"}</dd>
          </div>
          <div className="flex gap-1.5">
            <dt>{t("event")}:</dt>
            <dd className="text-slate-400">{d.event?.name ?? "—"}</dd>
          </div>
          <div className="flex gap-1.5">
            <dt>{t("created")}:</dt>
            <dd className="text-slate-400">{fmtDateTime(d.created_at, locale, tz)}</dd>
          </div>
        </dl>
        ...
        <section className="glass p-5 lg:col-span-2">
          ...
          <ul className="mt-4 space-y-3">
            ...
            {observations.length === 0 && (
              <li className="text-sm text-slate-500">{t("noObservations")}</li>
            )}
          </ul>
        </section>
            ...
            {history.map((h) => (
              <li key={h.id} className="text-xs">
                <p className="text-slate-300">
                  <span className="font-medium text-slate-200">{fieldLabel(h.field_changed)}</span>:{" "}
                  {renderVal(h.field_changed, h.old_value)} →{" "}
                  {renderVal(h.field_changed, h.new_value)}
                </p>
                <p className="text-slate-500">
                  {nameOf(h.changed_by)} · {fmtDateTime(h.created_at, locale, tz)}
                </p>
              </li>
            ))}
