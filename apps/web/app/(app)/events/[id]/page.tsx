...
export default async function EventDetailPage({
  ...
  const tabHref = (tb: 'demandas' | 'participantes') => (tb === 'demandas' ? `/events/${ev.id}` : `/events/${ev.id}?tab=participantes`)

  return (
    <div className="mx-auto max-w-3xl">
      <Link href="/events" className="text-sm text-slate-400 transition hover:text-white">← {t('back')}</Link>
