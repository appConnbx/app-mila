...
import { fmtDateTime as fmtDateTimeTz, fmtDate as fmtDateTz } from "@/lib/datetime";
...
export default async function EventDetailPage({
  ...
  const { data: evData } = await supabase
    .from("events")
    .select(
      "id, name, status, event_date, opened_at, closed_at, owner_id, owner:owner_id(full_name)",
    )
    .eq("id", id)
    .single();
  ...
  const { data: hRow } = await supabase
    ...
    .single();
  const tz =
    (hRow as unknown as { timezone: string | null } | null)?.timezone ?? "America/Sao_Paulo";
  ...
  const tabHref = (tb: "demandas" | "participantes") =>
    tb === "demandas" ? `/events/${ev.id}` : `/events/${ev.id}?tab=participantes`;

  return (
    <div className="mx-auto max-w-3xl">
      <Link href="/events" className="text-sm text-slate-400 transition hover:text-white">
        ← {t("back")}
      </Link>
      ...
      <div className="mt-5 glass p-5">
        ...
        <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2 text-xs text-slate-500 sm:grid-cols-4">
          <div>
            <dt>{t("date")}</dt>
            <dd className="text-slate-300">{fmtDate(ev.event_date)}</dd>
          </div>
          <div>
            <dt>{t("owner")}</dt>
            <dd className="text-slate-300">{ev.owner?.full_name ?? "—"}</dd>
          </div>
          <div>
            <dt>{t("opened")}</dt>
            <dd className="text-slate-300">{fmtDateTime(ev.opened_at)}</dd>
          </div>
          <div>
            <dt>{t("closed")}</dt>
            <dd className="text-slate-300">{fmtDateTime(ev.closed_at)}</dd>
          </div>
        </dl>
            ...
            <form action={addEventDemand} className="glass space-y-3 p-5">
              ...
              <div>
                ...
                <input
                  ...
                  name="title"
                  required
                  placeholder={tn("demandPlaceholder")}
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                <div>
                  <label htmlFor="responsible_id" className={labelCls}>
                    {tn("responsible")}
                  </label>
                  <select
                    id="responsible_id"
        ...
        <div className="mt-4 glass p-5">
          <ul className="space-y-2">
            ...
            {participants.length === 0 && (
              <li className="text-sm text-slate-500">{t("noParticipants")}</li>
            )}
          </ul>
          {amOwner && addable.length > 0 && (
            <form
              action={addParticipant}
              className="mt-3 flex gap-2 border-t border-surface-border pt-4"
            >
              <input type="hidden" name="event_id" value={ev.id} />
