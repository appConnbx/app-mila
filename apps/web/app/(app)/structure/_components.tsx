...
export async function StatusBadge({ active }: { active: boolean }) {
  ...
  return (
    <Badge variant={active ? "success" : "neutral"}>{active ? t("active") : t("inactive")}</Badge>
  );
}
    ...
    <Card title={t("admins")}>
      <ul className="space-y-1.5">
        {admins.map((a) => (
          <li
            key={a.id}
            className="flex items-center justify-between rounded-lg bg-slate-900/40 px-3 py-2 text-sm text-slate-200"
          >
    ...
    <Card title={t("people")}>
      <ul className="space-y-2">
        {people.map((p) => (
          <li
            key={p.id}
            className={`rounded-lg bg-slate-900/40 px-3 py-2 ${p.is_active ? "" : "opacity-60"}`}
          >
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
              {p.aliases.map((al) => (
                <span
                  key={al.id}
                  className="inline-flex items-center gap-1 rounded-full border border-surface-border bg-slate-800/60 px-2 py-0.5 text-[11px] text-slate-300"
                >
                  {al.alias}
                  <form action={removeAlias} className="inline">
                    <input type="hidden" name="id" value={al.id} />
                    <SubmitButton className="text-slate-500 transition hover:text-red-400">
                      ×
                    </SubmitButton>
                  </form>
                </span>
