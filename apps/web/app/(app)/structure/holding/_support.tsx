...
type Thread = {
  ok: boolean;
  ticket?: { title: string; description: string | null; status: string };
  messages?: Msg[];
};
...
export function ClientSupport({
  ...
  const rows = useMemo(() => {
    const base = tickets.filter((tk) =>
      filter === "open" ? tk.status !== "resolvido" : tk.status === "resolvido",
    );
  ...
  return (
    <div className="space-y-4">
      {flash && (
        <div
          className={`rounded-xl border px-4 py-3 text-sm ${flash === "ok" ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-300" : "border-rose-500/30 bg-rose-500/10 text-rose-300"}`}
        >
      ...
      <div className="inline-flex rounded-lg border border-white/10 bg-white/[0.03] p-1">
        <button
          ...
          className={`rounded-md px-3 py-1 text-sm transition ${filter === "open" ? "bg-white/10 text-white" : "text-slate-400 hover:text-white"}`}
        >
        ...
        <button
          ...
          className={`rounded-md px-3 py-1 text-sm transition ${filter === "closed" ? "bg-white/10 text-white" : "text-slate-400 hover:text-white"}`}
        >
            ...
            {rows.map((tk) => (
              ...
              >
                  ...
                  <div className="flex items-center gap-2">
                    {tk.unread && (
                      <span
                        ...
                        title={t("supportUnread")}
                      />
                    )}
                    <span className="font-medium text-slate-100">{tk.title}</span>
                  </div>
                  {tk.unread && (
                    <p className="mt-0.5 text-xs text-rose-300">{t("supportUnread")}</p>
                  )}
                </td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3 text-right">
                  <Button size="sm" variant="secondary" onClick={() => openDetail(tk.id)}>
                    {t("supportOpenBtn")}
                  </Button>
                </td>
              </tr>
        ...
        >
          ...
          >
            <div className="flex items-start justify-between">
              <h3 id="sup-modal-title" className="text-lg font-bold text-white">
                {t("supportNewTitle")}
              </h3>
              <button
                type="button"
            ...
            <form action={openSupportTicket} className="mt-4 space-y-3">
              <div>
                <label htmlFor="sup-title" className="block text-sm font-medium text-slate-300">
                  {t("supportSubject")}
                </label>
                <input
                  id="sup-title"
                  ...
                  className="mt-1 w-full rounded-lg border border-white/10 bg-slate-900/60 px-3 py-2 text-sm text-white outline-none placeholder:text-slate-500 focus:border-brand/60"
                />
              </div>
              <div>
                <label htmlFor="sup-desc" className="block text-sm font-medium text-slate-300">
                  {t("supportMessage")}
                </label>
                <textarea
                  id="sup-desc"
            ...
            {!loading && thread?.ok && (
              <>
                {thread.ticket?.status && (
                  <Badge
                    variant={
                      STATUS_VARIANT[thread.ticket.status as ClientTicket["status"]] ?? "info"
                    }
                    className="mt-2"
                  >
                ...
                <div className="mt-4 space-y-3">
                  {(thread.messages ?? []).length === 0 && (
                    <p className="text-sm text-slate-500">{t("supportThreadEmpty")}</p>
                  )}
                  {(thread.messages ?? []).map((m) => (
                    <div
                      key={m.id}
                      className={`rounded-lg p-3 ${m.from_support ? "bg-brand/10" : "ml-6 bg-slate-900/50"}`}
                    >
                      <p className="text-[11px] font-semibold text-slate-400">
                        {m.from_support ? t("supportFromSupport") : t("supportFromYou")} ·{" "}
                        {fmt(m.created_at)}
                      </p>
                ...
                {thread.ticket?.status !== "resolvido" && (
                  <div className="mt-5 space-y-3 border-t border-white/10 pt-4">
                    <form
                      action={clientReplyTicket}
                      className="space-y-2"
                      onSubmit={() => setTimeout(() => openDetail(openId), 400)}
                    >
                      ...
                      <textarea
                        ...
                        rows={2}
                        required
                        placeholder={t("supportReplyPh")}
                        className="w-full rounded-lg border border-white/10 bg-slate-900/60 px-3 py-2 text-sm text-white outline-none placeholder:text-slate-500 focus:border-brand/60"
                      />
                      <SubmitButton btnVariant="primary" btnSize="sm">
                        {t("supportReply")}
                      </SubmitButton>
                    </form>
                    <form
                      action={clientCloseTicket}
                      onSubmit={() => {
                        setTimeout(() => {
                          setOpenId(null);
                          setThread(null);
