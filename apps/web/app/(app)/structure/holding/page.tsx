...
import { Breadcrumb, Card, btnCls, inputCls } from "../_components";
...
export default async function HoldingPage({
  searchParams,
}: { searchParams: Promise<{ tab?: string; support?: string; cfg?: string }> }) {
  ...
  const { data } = await supabase
    .from("holdings")
    .select(
      "id, name, kind, legal_name, tax_id, contact_email, phone, timezone, language, logo_url",
    )
    .eq("id", holdingId)
    .single();
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <div className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3">
              ...
              <p className="mt-1 text-lg font-bold text-white">
                {lic.is_unlimited ? t("licenseUnlimited") : lic.seat_limit}
              </p>
            </div>
            <div className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3">
              <p className="text-[11px] uppercase tracking-wide text-slate-500">
            ...
            <div className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3">
              ...
              <p className="mt-1 text-lg font-bold text-emerald-300">
                {lic.is_unlimited ? "∞" : (lic.available ?? 0)}
              </p>
            </div>
          </div>
        ) : (
        ...
        <Card title={t("holdingData")}>
          ...
          <div>
            ...
            <input
              id="legal_name"
              name="legal_name"
              defaultValue={h.legal_name ?? ""}
              className={`mt-1 ${inputCls}`}
            />
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label htmlFor="tax_id" className={labelCls}>
                {t("taxId")}
              </label>
              <input
                id="tax_id"
                name="tax_id"
                defaultValue={h.tax_id ?? ""}
                className={`mt-1 ${inputCls}`}
              />
            </div>
            <div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <select
                ...
                id="timezone"
                name="timezone"
                defaultValue={h.timezone ?? "America/Sao_Paulo"}
                className={`mt-1 ${inputCls}`}
              >
                {TIMEZONES.map((tz) => (
  ...
  const supportNode = (
    <ClientSupport
      tickets={tickets}
      manualHref={`/manual/${manualSeg}`}
      flash={support === "ok" ? "ok" : support ? "err" : undefined}
    />
  );
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <Breadcrumb
        items={[{ href: "/structure", label: t("title") }, { label: t("holdingMgmt") }]}
      />
