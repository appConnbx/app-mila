...
export default async function OrgPage({ params }: { params: Promise<{ orgId: string }> }) {
  ...
  return (
    <div>
      <Breadcrumb items={[{ href: '/structure', label: t('title') }, { label: org.name }]} />
      ...
      <div className="mt-6 space-y-5">
        ...
        {/* 1) Configurações */}
        <StructureSettings kind="organization" id={orgId} name={org.name} isActive={org.is_active} redirectAfterDelete="/structure" />
        ...
        <Card title={t('areas')}>
          <ul className="space-y-1.5">
            {areas.map((a) => (
              <li key={a.id}>
                <Link
                  href={`/structure/org/${orgId}/area/${a.id}`}
