...
import { UsersManager, type HoldingUser } from './users/_users-manager'
            ...
            {orgs.map((o) => (
              <tr key={o.id} className={`border-b border-white/5 last:border-0 transition hover:bg-white/[0.025] ${o.is_active ? '' : 'opacity-55'}`}>
                <td className="px-4 py-3">
                  <Link href={`/structure/org/${o.id}`} className="font-semibold text-slate-100 transition hover:text-brand">{o.name}</Link>
                </td>
                <td className="px-4 py-3"><StatusBadge active={o.is_active} /></td>
                <td className="px-4 py-3 text-right">
                  <Link href={`/structure/org/${o.id}`} className="text-xs font-semibold text-brand transition hover:text-brand-500">{t('openFolder')} →</Link>
                </td>
              </tr>
            ))}
            {orgs.length === 0 && (
