...
export default async function WelcomeMemberPage() {
  ...
  const [{ data: hData }, { data: pRows }] = await Promise.all([
    ...
    supabase.from('people').select('full_name').eq('auth_user_id', user?.id ?? '').eq('holding_id', holdingId).limit(1),
  ])
  const h = hData as unknown as Holding | null
  if (!h) redirect('/dashboard')
  const firstName = ((pRows as unknown as { full_name: string }[] | null)?.[0]?.full_name ?? user?.email?.split('@')[0] ?? '')
    .split(' ')[0]
    ...
    <div className={`mx-auto max-w-2xl ${isFamily ? 'theme-family' : ''}`}>
        ...
        <ul className="mt-3 space-y-2">
          {bullets.map((b, i) => (
            <li key={i} className="flex gap-2 text-sm text-slate-200">
              <svg viewBox="0 0 20 20" width="16" height="16" fill="none" aria-hidden className="mt-0.5 shrink-0 text-brand">
      ...
      <div className="mt-3 grid gap-4 sm:grid-cols-2">
        ...
        <div className="glass flex flex-col p-5">
          ...
          <a href="https://www.appmila.co" target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center justify-center rounded-lg border border-white/15 px-4 py-2 text-sm font-medium text-slate-200 transition hover:bg-white/10">
            {t('mobileCta')}
          </a>
        </div>
        {/* Agente Windows */}
        <div className="glass flex flex-col p-5">
          ...
          <a href="/api/agent/download" className="mt-4 inline-flex items-center justify-center gap-2 rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-brand-500">
      ...
      {/* Começar */}
      <form action={finishMemberOnboarding} className="mt-8 flex flex-wrap items-center gap-4 border-t border-white/10 pt-5">
