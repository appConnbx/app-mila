...
export function ProfileEditor({
  ...
  email,
  initial,
}: { uid: string; name: string; email: string; initial: Initial }) {
        ...
        <div>
          ...
          <div className="mt-1 flex flex-wrap gap-1.5">
            {skills.map((s) => (
              <span
                key={s}
                className="inline-flex items-center gap-1 rounded-md bg-brand/15 px-2 py-0.5 text-[12px] font-semibold text-cyan-200"
              >
                {s}
                <button
                  type="button"
