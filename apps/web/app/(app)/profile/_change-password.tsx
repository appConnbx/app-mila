  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    start(async () => {
      ...
      if (r.ok) {
        ...
        const text =
          r.error === "short" ? l.errShort : r.error === "mismatch" ? l.errMismatch : l.errFail;
  ...
  return (
    <section className="glass mt-6 p-6">
      ...
      <form onSubmit={onSubmit} className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        {msg && (
          <p
            className={`sm:col-span-2 rounded-lg border px-3 py-2 text-sm ${msg.ok ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-300" : "border-rose-500/30 bg-rose-500/10 text-rose-300"}`}
          >
            {msg.text}
          </p>
        )}
