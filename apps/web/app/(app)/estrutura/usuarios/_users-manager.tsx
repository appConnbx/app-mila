          ...
          <tbody>
            {filtered.map((u) => (
              <tr key={u.id} className={`border-b border-white/5 last:border-0 ${u.is_active ? '' : 'opacity-55'}`}>
                ...
                <td className="px-4 py-3">
                  {u.teams.length ? (
                    <div className="flex flex-wrap gap-1">
                      {u.teams.map((tm) => (
                        <span key={tm} className="rounded-md border border-white/10 bg-white/[0.05] px-2 py-0.5 text-[11px] text-slate-300">{tm}</span>
                      ))}
                    </div>
                  ) : (
                ...
                <td className="px-4 py-3">
                  <Badge variant={u.is_active ? 'success' : 'neutral'}>{u.is_active ? t('active') : t('inactive')}</Badge>
                </td>
                <td className="px-4 py-3">
        ...
        <div className="fixed inset-0 z-50 flex justify-end" role="dialog" aria-modal="true">
          ...
          <div className="glass glow-top relative h-full w-full max-w-md overflow-y-auto p-6 sm:rounded-l-2xl">
            <div className="flex items-start justify-between">
              ...
              <button onClick={() => setOpenId(null)} className="rounded-lg p-1.5 text-slate-400 transition hover:bg-white/10 hover:text-white" aria-label={t('close')}>✕</button>
            </div>
            ...
            <form action={updatePerson} className="mt-5 space-y-3">
              ...
              <div>
                ...
                <input name="full_name" defaultValue={open.full_name} className={`mt-1 ${fieldClasses}`} required />
              </div>
              <div>
                <label className={labelClasses}>{t('emailOptional')}</label>
                <input name="email" type="email" defaultValue={open.email ?? ''} className={`mt-1 ${fieldClasses}`} />
              </div>
              <div>
                <label className={labelClasses}>{t('roleOptional')}</label>
                <input name="role_title" defaultValue={open.role_title ?? ''} className={`mt-1 ${fieldClasses}`} />
              </div>
