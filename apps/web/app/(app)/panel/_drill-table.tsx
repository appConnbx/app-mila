  ...
  const numCells = (r: StatRow) => (
    <>
      <td className="px-3 py-2.5">
        {r.streak > 0 ? (
          <span className="font-semibold text-amber-300">🔥 {r.streak}</span>
        ) : (
              ...
              <FragmentRows key={area.id}>
                ...
                >
                  <td className="px-3 py-2.5 font-semibold text-slate-100">
                    <span
                      className={`mr-2 inline-block text-xs text-brand transition-transform ${areaOpen ? "rotate-90" : ""}`}
                    >
                      ▶
                    </span>
                    {area.name}
                      ...
                      <FragmentRows key={team.id}>
                        ...
                        >
                          <td className="px-3 py-2.5 pl-9 font-medium text-slate-200">
                            <span
                              className={`mr-2 inline-block text-xs text-brand/80 transition-transform ${teamOpen ? "rotate-90" : ""}`}
                            >
                              ▶
                            </span>
                            {team.name}
                        ...
                        {teamOpen && people.length === 0 && (
                          <tr className="border-b border-white/5 bg-white/[0.04]">
                            <td
                              colSpan={cols.length}
                              className="px-3 py-2.5 pl-16 text-xs text-slate-500"
                            >
                              {t("noData")}
                            </td>
                          </tr>
