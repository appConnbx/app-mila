...
import { type DrillArea, DrillTable } from "./_drill-table";
import { type StatRow, StatTable } from "./_stat-table";
      ...
      {rows.map((r, i) => {
        ...
        return (
          <div key={r.id} className="glass flex items-center gap-3 px-4 py-3">
            <span
              className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-sm font-bold ${!fromBottom && i === 0 ? "bg-brand text-slate-950" : "bg-slate-800 text-slate-300"}`}
            >
              {rank}
            </span>
