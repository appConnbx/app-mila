...
export function Avatar({
  ...
  size = "md",
  src,
}: { name: string; size?: "sm" | "md" | "lg"; src?: string | null }) {
  const sizes = {
...
export function Tag({
  children,
  tone = "brand",
}: { children: ReactNode; tone?: "brand" | "danger" | "auto" }) {
  const tones = {
...
export function DeadlineBar({
  ...
  const pct = overdue
    ? 100
    : Math.max(4, Math.min(100, Math.round(((now - start) / Math.max(1, end - start)) * 100)));
...
export function EmptyState({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div className={cn("px-5 py-10 text-center text-sm text-slate-500", className)}>{children}</div>
  );
}
