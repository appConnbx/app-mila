...
import { buttonClasses, type Size, type Variant } from '@/components/ui'
...
export function SubmitButton({
  ...
  if (mode === 'overlay') {
    return (
      <button type="submit" disabled={busy} aria-busy={pending} className={cn('relative', base)} {...props}>
  ...
  return (
    <button type="submit" disabled={busy} aria-busy={pending} className={cn('relative', base)} {...props}>
      <span className={cn('inline-flex items-center justify-center gap-1.5', pending && 'invisible')}>{children}</span>
      {pending && (
