        ...
        >
          ...
          >
            <div className="flex items-center justify-between">
              <h2 id="new-demand-title" className="text-xl font-bold text-white">{l.title}</h2>
              <button
                type="button"
            ...
            <form onSubmit={onSubmit} className="mt-5 space-y-4">
              <div>
                <label htmlFor="nd-title" className={labelCls}>{l.demand}</label>
                <input id="nd-title" name="title" required placeholder={l.demandPlaceholder} className={inputCls} />
              </div>

              <div>
                <label htmlFor="nd-description" className={labelCls}>{l.description}</label>
                <textarea id="nd-description" name="description" rows={3} className={inputCls} />
              </div>

              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label htmlFor="nd-responsible" className={labelCls}>{l.responsible}</label>
                  <select id="nd-responsible" name="responsible_id" required defaultValue="" className={inputCls}>
                    <option value="" disabled>{l.selectPlaceholder}</option>
                    {people.map((p) => <option key={p.id} value={p.id}>{p.full_name}</option>)}
                  </select>
                </div>
                <div>
                  <label htmlFor="nd-priority" className={labelCls}>{l.priority}</label>
                  <select id="nd-priority" name="priority" defaultValue="media" className={inputCls}>
              ...
              <div className={isFamily ? '' : 'grid gap-4 sm:grid-cols-2'}>
                <div>
                  <label htmlFor="nd-due" className={labelCls}>{l.due}</label>
                  <input id="nd-due" name="due_date" type="date" className={inputCls} />
                </div>
                {!isFamily && (
                  <div>
                    <label htmlFor="nd-event" className={labelCls}>{l.event}</label>
              ...
              <div>
                <label htmlFor="nd-visibility" className={labelCls}>{l.visibility}</label>
                <select id="nd-visibility" name="visibility" defaultValue="private" className={inputCls}>
