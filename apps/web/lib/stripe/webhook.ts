...
const toIso = (epochSecs?: number | null) =>
    ...
    from: (t: string) => {
      select: (c: string) => {
        eq: (
          ...
          limit: (n: number) => Promise<{ data: { id: string; holding_id: string }[] | null }>;
        };
      };
    };
  };
...
export async function processStripeEvent(admin: Admin, event: Any): Promise<StripeResult> {
  ...
  if (type === "checkout.session.completed") {
    if (obj.mode !== "subscription")
      return { status: "ignorado", message: "sessão não-assinatura" };
    ...
    try {
      ...
      console.warn(
        "stripe: falha ao ler período da assinatura",
        subId,
  ...
  if (type === "charge.dispute.created") {
    ...
    if (!customerId && chargeId) {
      try {
        ...
        console.warn(
          "stripe: falha ao resolver charge da disputa",
          chargeId,
          e instanceof Error ? e.message : e,
        );
      }
