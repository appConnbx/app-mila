...
function normalize(payload: Any) {
  ...
  return {
    ...
    productId:
      pick(data, 'product.id') != null ? String(pick(data, 'product.id')) : undefined,
    offerCode:
      pick<string>(data, 'purchase.offer.code') ??
      pick<string>(data, 'offer.code') ??
      undefined,
    subscriberCode:
...
async function resolvePlanId(admin: Admin, productId?: string, offerCode?: string): Promise<string | null> {
  ...
  const { data } = (await q) as { data: { id: string; external_offer_code: string | null }[] | null }
  const rows = data ?? []
...
async function findSubscriptionByCode(admin: Admin, code?: string, transaction?: string) {
  const sb = admin as unknown as {
    from: (t: string) => {
      select: (c: string) => { or: (f: string) => { limit: (n: number) => Any }; eq: (c: string, v: string) => { limit: (n: number) => Any } }
    }
  }
  if (code) {
    const { data } = (await sb.from('subscriptions').select('id, holding_id').eq('external_subscription_code', code).limit(1)) as {
      data: { id: string; holding_id: string }[] | null
    }
    if (data?.[0]) return data[0]
  }
  if (transaction) {
    const { data } = (await sb.from('subscriptions').select('id, holding_id').eq('external_transaction', transaction).limit(1)) as {
      data: { id: string; holding_id: string }[] | null
    }
    if (data?.[0]) return data[0]
  }
...
export async function processHotmartEvent(admin: Admin, payload: Any): Promise<HotmartResult> {
  ...
  if (f.event === 'PURCHASE_APPROVED' || f.event === 'PURCHASE_COMPLETE') {
    ...
    if (!planId) return { status: 'ignorado', message: `plano não mapeado (product=${f.productId}, offer=${f.offerCode})` }
  ...
  if (f.event === 'PURCHASE_REFUNDED' || f.event === 'PURCHASE_CHARGEBACK' || f.event === 'PURCHASE_PROTEST') {
    const sub = await findSubscriptionByCode(admin, f.subscriberCode, f.transaction)
    if (!sub) return { status: 'ignorado', message: 'assinatura não encontrada' }
    const sb = admin as unknown as { from: (t: string) => { update: (v: Record<string, unknown>) => { eq: (c: string, v: string) => Promise<unknown> } } }
    await sb.from('subscriptions').update({ status: 'suspended', updated_at: new Date().toISOString() }).eq('id', sub.id)
