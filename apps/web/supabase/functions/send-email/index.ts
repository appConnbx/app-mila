...
Deno.serve(async (req) => {
  ...
  let evt: { user: { email: string }; email_data: { token_hash: string; email_action_type: string; redirect_to?: string } }
  try {
    const wh = new Webhook(HOOK_SECRET.replace('v1,whsec_', ''))
    evt = wh.verify(raw, Object.fromEntries(req.headers)) as typeof evt
  } catch {
    return new Response(JSON.stringify({ error: 'assinatura inválida' }), { status: 401, headers: { 'Content-Type': 'application/json' } })
  }
  ...
  if (!res.ok) {
    ...
    return new Response(JSON.stringify({ error: 'falha no envio', detail }), { status: 500, headers: { 'Content-Type': 'application/json' } })
  }
