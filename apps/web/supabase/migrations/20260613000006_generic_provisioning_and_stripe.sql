-- Provisionamento genérico de assinatura (reaproveitável por qualquer provider).
-- Hotmart passa a delegar a esta função; Stripe usa a mesma via provider='stripe'.
...
begin
  ...
  select account_kind, coalesce(included_users, max_users)
    into v_kind, v_seats from public.plans where id = p_plan_id;
...
create or replace function public.provision_from_hotmart(
  p_plan_id uuid, p_external_subscription_code text, p_buyer_email text, p_buyer_name text,
  p_auth_user_id uuid, p_current_period_end timestamptz default null, p_external_transaction text default null
) returns jsonb language sql security definer set search_path to 'public','app' as $$
  select public.provision_subscription(p_plan_id, 'hotmart'::app.billing_provider, p_external_subscription_code,
    p_buyer_email, p_buyer_name, p_auth_user_id, p_current_period_end, p_external_transaction);
$$;

-- Customer Stripe da holding do usuário logado (para abrir o Billing Portal).
create or replace function public.my_stripe_customer() returns text
language sql stable security definer set search_path to 'public','app' as $$
  select s.external_transaction
  from public.people pe
  join public.subscriptions s on s.holding_id = pe.holding_id and s.provider = 'stripe'
  where pe.auth_user_id = auth.uid() and s.external_transaction is not null
  order by s.created_at desc limit 1;
$$;
grant execute on function public.my_stripe_customer() to authenticated;
